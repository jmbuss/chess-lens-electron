...
<template>
  <RangeCalendarCell
    :class="cn('relative h-9 w-9 p-0 text-center text-sm focus-within:relative focus-within:z-20 [&:has([data-selected]:not([data-outside-view]))]:bg-accent first:[&:has([data-selected]:not([data-outside-view]))]:rounded-l-md last:[&:has([data-selected]:not([data-outside-view]))]:rounded-r-md [&:has([data-selected][data-selection-end])]:rounded-r-md [&:has([data-selected][data-selection-start])]:rounded-l-md', props.class)"
